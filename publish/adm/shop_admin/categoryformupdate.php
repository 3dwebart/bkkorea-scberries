...
/* BIGIN :: 여분필드를 활용한 메인 메뉴 이미지 */
@mkdir(G5_DATA_PATH."/menuimg", G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH."/menuimg", G5_DIR_PERMISSION);

$menu_img      = $_FILES['ca_1']['tmp_name'];
$menu_img_name = $_FILES['ca_1']['name'];

$menu_img_del = $_POST['menu_img_del'];
if ($menu_img_del) {
	@unlink(G5_DATA_PATH."/menuimg/$ca_id");
}

//파일이 이미지인지 체크합니다.
if( $menu_img || $menu_img_name ) {

	if( !preg_match('/\.(gif|jpe?g|bmp|png)$/i', $menu_img_name) ){
		alert("이미지 파일만 업로드 할수 있습니다.");
	} else {
		$ca_1 = 'menuimg/'.$ca_id;
	}

	$timg = @getimagesize($menu_img);
...
{
	if ($_FILES['ca_1']['name']) {
		upload_file($_FILES['ca_1']['tmp_name'], $ca_id, G5_DATA_PATH."/menuimg");
	}
