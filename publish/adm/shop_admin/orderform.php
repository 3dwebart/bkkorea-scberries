		...
		<section id="anc_sodr_paymo" class="compare_right">
			...
			<div class="tbl_frm01">
				...
				<?php
					...
					$dlcomp_info = array();
					for($i = 0; $i < count($dlcomp) - 1; $i++) {
						$dlcomp_info[$i] = explode('^', $dlcomp[$i]);
					}
				?>
				<tr>
					...
					<td class="delevery">
						...
						<select name="od_delivery_company" id="od_delivery_company_select">
							<?php
							for($i = 0; $i < count($dlcomp_info); $i++ ) {
								$dlcomp_name = $dlcomp_info[$i][0];
							?>
							<option value="<?php echo $dlcomp_name; ?>">
								<?php echo $dlcomp_name; ?>
							</option>
							<?php } ?>
							<option value="order_delivary_company">기타</option>
						</select>
				...
				<script>
					(function($) {
						...
						1. 저장된 값이 있는지 검색
						2. 없으면 신규 입력폼은 없음
						3. 신규작성시 기타배송으로 처리되어 텍스트 입력폼 생성
						4. 저장된 값이 등록된 배송회사와 일치하는지 검색
						5. 일치하면 해당 옵션이 selected
						6. 일치하는 option 이 없으면
						6-1. 기타 옵션(value가 order_delivary_company 인 option)이 활성화되고 DB 값을 input에 넣어서 생성
						6-2. 다른 옵션 성택후 다시 키타 옵션 선택시 6-1 반복함
						*/
						var od_delivery_company = "<?php echo $od['od_delivery_company']; ?>";
						var tmp_dlcomp = "<?php echo G5_DELIVERY_COMPANY; ?>";
						var dlcomp = tmp_dlcomp.split(')');
						var dlcomp_arr = new Array();
						var delevery_html = '<input type="text" name="od_delivery_company" id="od_delivery_company" value="<?php echo $od['od_delivery_company']; ?>" class="frm_input">';
						var indesOfValue = false;
						for (var i = 0; i < dlcomp.length - 1; i++) {
							dlcomp_arr[i] = dlcomp[i].replace('(', '').split('^');
						}

						if(tmp_dlcomp.indexOf(od_delivery_company) == -1) {
							jQuery('#od_delivery_company_select option[value="order_delivary_company"]').prop('selected', true);
							//jQuery('.delevery').append(delevery_html);
						} else {
							jQuery('#od_delivery_company_select option').each(function() {
								if(jQuery(this).val() == od_delivery_company) {
									jQuery(this).prop('selected', true);
									indesOfValue = true;
									return indesOfValue;
								}
							});
							jQuery('#od_delivery_company').remove();
						}
						console.log('return :: ' + indesOfValue);

						jQuery('#od_delivery_company_select').on('change', function() {
							if($(this).val() == 'order_delivary_company') {
								jQuery('.delevery').append(delevery_html);
								if(indesOfValue == true) {
									jQuery('#od_delivery_company').val('');
								}
							} else {
								jQuery('#od_delivery_company').remove();
							}
...
function form_submit(f)
{
	...
	if(status == "취소" || status == "반품" || status == "품절") {
		...
		var chk_cnt = $ct_chk.length; // jQuery 3.x is gone size( )
		var chked_cnt = $ct_chk.filter(":checked").length; // jQuery 3.x is gone size( )
