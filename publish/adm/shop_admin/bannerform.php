...
<div class="tbl_frm01 tbl_wrap">
	...
	<tr>
		...
		<td>
			...
			<select name="bn_position" id="bn_position">
				...
				<option value="메인꾸밈" <?php echo get_selected($bn['bn_position'], '메인꾸밈'); ?>>메인꾸밈</option>
		</select>
		</td>
	</tr>
	<tr style="display: none;">
		...
		<td>
			<select name="bn_item_type" id="bn_item_type">
				...
				<option value="2" <?php echo get_selected($bn['bn_item_type'], '2'); ?>>
					추천상품
				</option>
				<option value="3" <?php echo get_selected($bn['bn_item_type'], '3'); ?>>
					신상품
				</option>
				<option value="4" <?php echo get_selected($bn['bn_item_type'], '4'); ?>>
					인기상품
				</option>
	...
	<tr style="display: none;">
		<th scope="row"><label for="bn_item_cate">노출 카테고리</label></th>
		<td>
			<select name="bn_item_cate" id="bn_item_cate">
				<option value="0" <?php echo get_selected($bn['bn_item_cate'], '0'); ?>>
					카테고리 선택
				</option>
				<?php
					$bn_item_cate_sql = "SELECT * FROM {$g5['g5_shop_category_table']}";
					$bn_item_cate_res = sql_query($bn_item_cate_sql);
					while ($row = sql_fetch_array($bn_item_cate_res)) {
				?>
				<option value="<?php echo $row['ca_id']; ?>" <?php echo get_selected($bn['bn_item_cate'], "{$row['ca_id']}"); ?>>
...
<script>
(function($) {
	var posVal = jQuery('#bn_position option:selected').val();
	var bn_item_type = jQuery('#bn_item_type option:selected').val();
	var bn_item_cate = jQuery('#bn_item_cate option:selected').val();
	if(posVal == '메인꾸밈') {
		jQuery('#bn_item_type').closest('tr').show();
	} else {
		jQuery('#bn_item_type').closest('tr').hide();
	}
	if(posVal == '메인꾸밈' && bn_item_type == 0) {
		jQuery('#bn_item_cate').closest('tr').show();
	} else {
		jQuery('#bn_item_cate').closest('tr').hide();
	}
	jQuery(document).on('change', '#bn_position', function() {
		posVal = jQuery(this).val();
		bn_item_type = jQuery('#bn_item_type option:selected').val();

		if(posVal == '메인꾸밈') {
			console.log(bn_item_type);
			jQuery('#bn_item_type').closest('tr').show();
			if(bn_item_type == 0) {
				jQuery('#bn_item_cate').closest('tr').show();
			} else {
	...
	jQuery(document).on('change', '#bn_item_type', function() {
		bn_item_type = jQuery(this).val();
		if(bn_item_type == 0) {
			jQuery('#bn_item_cate').closest('tr').show();
		} else {
			jQuery('#bn_item_cate').closest('tr').hide();
