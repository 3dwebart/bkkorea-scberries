...
$bn_item_type = $_POST['bn_item_type'];
$bn_item_cate = $_POST['bn_item_cate'];

if ($w=="")
{
    if (!$bn_bimg_name) alert('배너 이미지를 업로드 하세요.');

    sql_query(" AFTER TABLE {$g5['g5_shop_banner_table']} auto_increment=1 ");

    $sql = " INSERT INTO {$g5['g5_shop_banner_table']}
                SET bn_alt        = '$bn_alt',
                    bn_url        = '$bn_url',
                    bn_item_type  = '$bn_item_type',
                    bn_item_cate  = '$bn_item_cate',
                    bn_device     = '$bn_device',
...
if ($w == "" || $w == "u")
{
    if ($_FILES['bn_bimg']['name']) {
        upload_file($_FILES['bn_bimg']['tmp_name'], $bn_id, G5_DATA_PATH."/banner");
    }
