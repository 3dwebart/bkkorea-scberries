...
<article id="bo_v" style="width:<?php echo $width; ?>">
	...
	<table class="table info-table" summary="<?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?>의 정보 테이블 입니다.">
		<caption class="sound_only"><?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?>의 정보 테이블</caption>
		<colgroup>
			<col width="10%" />
			<col width="40%" />
			<col width="10%" />
		...
		<tbody>
			...
			<tr>
				...
				<th>조회수</th>
				<td><strong><i class="fa fa-eye" aria-hidden="true"></i> <?php echo number_format($view['wr_hit']) ?>회</strong></td>
			</tr>
		</tbody>
	</table>
    ...
    <section id="bo_v_atc">
        ...
        <div class="view-title text-center">
	        <h2 class="recipe"><span class="d-inline-block">RECIPE</span></h2>
	        <h3 class="view-title-subject"><span class="d-inline-block"><?php echo $view['wr_subject']; ?></span></h3>
        </div>
        ...
        <!-- 본문 내용 시작 { -->
        <div class="info-title">재료</div>
        <div class="mt-1 mb-4 info-content"><?php echo $view['wr_1']; ?></div>
        <div class="info-title">Recipe</div>
        <?php
	        $code=preg_replace('/\n$/','',preg_replace('/^\n/','',preg_replace('/[\r\n]+/',"\n",$view['content'])));
			$content_arr = explode("\n",$code);
        ?>
        <div id="bo_v_con" class="mt-1 mb-4 info-content">
	        <ol>
		        <?php for($i = 0; $i < count($content_arr); $i++) { ?>
		        <li><?php echo(preg_replace('<br>', '', $content_arr[$i])); ?></li>
		        <?php } ?>
	        </ol>
        ...
        <?php
	        if(!empty($view['wr_2'])) {
		        $item_sql = "SELECT it_id, it_name, it_img1 FROM {$g5['g5_shop_item_table']} WHERE it_id = '{$view['wr_2']}'";
		        $item_row = sql_fetch($item_sql);
        ?>
        <div class="row">
	        <div class="col-2 border-box pt-3">
		        <a href="<?php echo G5_SHOP_URL.'/item.php?it_id='.$item_row['it_id']; ?>" class="d-block">
			        <img src="<?php echo G5_DATA_URL.'/item/'.$item_row['it_img1']; ?>" alt="$item_row['it_name']" class="img-fluid" />
			        <strong><span class="d-block pt-3"><?php echo $item_row['it_name']; ?></span></strong>
			        <span class="d-flex space-beetween pb-3">구매하러 바로 가기<span><i class="fa fa-arrow-right"></i></span></span>
		        </a>
	        </div>
        </div>
        <?php
