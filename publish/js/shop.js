...
$(function() {
    ...
    $(document).on("keyup", "select.it_option", function(e) {
        var sel_count = $("select.it_option").length; // jQuery 3.x is gone size( )
        ...
        switch(mode) {
            ...
            case "삭제":
                if(confirm("선택하신 옵션항목을 삭제하시겠습니까?")) {
                    ...
                    if($("#sit_sel_option .sit_spl_list").length > 0) { // jQuery 3.x is gone size( )
                        // 선택옵션이 하나이상인지
                        if($el.hasClass("sit_opt_list")) {
                            if($(".sit_opt_list").length <= 1) { // jQuery 3.x is gone size( )
                                del_exec = false;
                            }
                        }
                    }
    ...
    if($("#sit_sel_option > ul").length < 1) {
        ...
        if(type) {
            if($("#sit_sel_option .sit_spl_list").length > 0) { // jQuery 3.x is gone size( )
                ...
                if($("#sit_sel_option .sit_opt_list").length > 0) { // jQuery 3.x is gone size( )
